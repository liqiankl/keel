...
const DENSITY_KEY = "keel-density";
...
const SECTIONS: SectionMeta[] = [
  ...
  { id: "workspace",     label: "Workspace",     icon: Building2, description: "Name, branding, and slug" },
  { id: "members",       label: "Members",       icon: Users,     description: "Manage access and roles" },
              ...
              if (s.comingSoon) {
                return (
                  ...
                  >
                    ...
                    <span className="ml-auto text-[9px] font-semibold uppercase tracking-wide text-[var(--color-text-muted)] border border-[var(--color-border-subtle)] px-1.5 py-0.5 rounded-full">
                      v2
                    </span>
                  </div>
                );
...
function ProfileSection() {
  ...
  }, [name, email, color, currentUser.name, currentUser.email, currentUser.avatarColor]);
  ...
  function handleSave() {
    ...
    updateCurrentUser({ name: name.trim(), email: trimmedEmail, avatarColor: color });
    setSaved(true);
    setDirty(false);
    setTimeout(() => setSaved(false), 2000);
  }

  // Derive live initials from the name input
  const liveInitials = deriveInitials(name || currentUser.name);

  // Activity stats from real store data
  const myRequests = requests.filter((r) => r.submittedBy === currentUser.name);
  const activeCount  = myRequests.filter((r) => r.status !== "archived").length;
  const totalComments = requests.flatMap((r) => r.comments)
  ...
  return (
    <>
      ...
      <SettingsCard>
        ...
        <div className="flex items-center gap-4 px-5 py-5 border-b border-[var(--color-border-subtle)]">
          {/* Live avatar preview */}
          <div
          ...
          <div className="flex-1 min-w-0">
            ...
            />
            <p className="text-xs text-[var(--color-text-muted)] mt-1">{email || currentUser.email}</p>
          </div>
          <span className="inline-flex items-center gap-1.5 h-6 px-2.5 rounded-full text-xs font-medium bg-[var(--color-brand-subtle)] text-[var(--color-brand)] flex-shrink-0">
            <Shield size={11} />
        ...
        <div className="px-5 py-4">
          <div className="flex items-start justify-between gap-4">
            <div className="flex-1 min-w-0">
              ...
              <p className="text-xs text-[var(--color-text-muted)] mt-0.5">Used for notifications and login</p>
            </div>
            <div className="flex-shrink-0 flex flex-col items-end gap-1">
              ...
              />
              {emailError && (
                <p className="text-[11px] text-[var(--color-danger)]">{emailError}</p>
              )}
            </div>
          </div>
        </div>
      ...
      <SettingsCard>
        <div className="px-5 py-3 border-b border-[var(--color-border-subtle)]">
          <p className="text-sm font-medium text-[var(--color-text-primary)]">Your activity</p>
          <p className="text-xs text-[var(--color-text-muted)] mt-0.5">Derived from your contributions across all teams</p>
        </div>
        <div className="grid grid-cols-3 divide-x divide-[var(--color-border-subtle)]">
          {[
            { label: "Requests submitted", value: myRequests.length },
            ...
            <div key={label} className="flex flex-col items-center py-4 px-3 text-center">
              ...
              <span className="text-[11px] text-[var(--color-text-muted)] mt-0.5">{label}</span>
            </div>
          ))}
        </div>
      </SettingsCard>
    ...
    <>
      <SettingsCard>
        <SettingsRow label="Workspace name" description="Visible to all members">
          <SettingsInput value={name} onChange={setName} />
        </SettingsRow>

        <SettingsRow label="Slug" description="Used in shared links">
          <div className="flex items-center gap-2">
            <SettingsInput value={workspace.slug} readOnly />
            <button
              onClick={copySlug}
...
function MembersSection() {
  const members           = useAppStore((s) => s.workspace.members);
  const currentUser       = useAppStore((s) => s.workspace.currentUser);
            ...
            >
              ...
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-1.5">
                  ...
                  {isCurrentUser && (
                    <span className="text-[10px] text-[var(--color-text-muted)] bg-[var(--color-bg-elevated)] px-1.5 py-0.5 rounded-full border border-[var(--color-border-subtle)]">
                      you
                    </span>
                  )}
                </div>
          ...
          {ROLES.map((role) => (
            ...
            >
              ...
              <div>
                ...
                <p className="text-[10px] text-[var(--color-text-muted)]">{ROLE_DESC[role]}</p>
              </div>
            </button>
          ))}
        </div>
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="rounded-xl border border-[var(--color-border-subtle)] bg-[var(--color-bg-surface)] px-6 py-8 text-center">
        ...
        <span className="mt-4 inline-block text-[10px] font-semibold uppercase tracking-widest text-[var(--color-text-muted)] border border-[var(--color-border-subtle)] px-3 py-1 rounded-full">
          Coming soon
        </span>
      </div>
...
const UPCOMING_INTEGRATIONS = [
  ...
  { name: "Jira",          icon: Link2,         description: "Sync epics and issues from your Jira projects" },
  { name: "Linear",        icon: Link2,         description: "Import issues and keep status in sync" },
];
...
function AppearanceSection() {
  ...
  // Read density from localStorage, default comfortable
  ...
  function handleThemeSelect(t: "dark" | "light" | "system") {
    setTheme(t); // applies immediately — updates data-theme + localStorage
  }
  ...
  function handleSave() {
    // Theme and density are already applied live; save just gives confirmation feedback
    setSaved(true);
    setTimeout(() => setSaved(false), 2000);
  }
...
function ThemePreview({ variant }: { variant: "dark" | "light" | "system" }) {
  ...
  const isDark = variant === "dark" || (variant === "system" && systemDark);
  return (
    <div
      className={cn(
    ...
    >
      ...
      <div className="flex h-full">
        <div
          className={cn("w-6 h-full", isDark ? "bg-[#202027]" : "bg-[#ffffff]")}
        >
          {[0,1,2].map((i) => (
            <div
              key={i}
